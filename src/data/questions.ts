...
Systemy pozycyjne bazują na kolejności, pozycji znaków, nadając wagi każdemu w zależności od
pozycji znaku.​
Przykłady to 123 w systemie 10 lub 1011 w systemie dwójkowym, które jest równe 1 + 2 + 0 + 8 =
11 w systemie dziesiętnym. W informatyce używane są też systemy bazujące na wielokrotnościach 2
- ósemkowy oraz szesnastkowy - gdzie znaki z wagą od 10 do 16 wyrażane są kolejnymi litarami
alfabetu.
Systemy addytywne bazują na sumie wartości znaków, gdzie ich kolejność nie ma znaczenia, a jest
kwestią konwencji.​
Przykład to system pierwotny - liczenie zapałek/słupków lub system rzymski - np. C XL V = V XL C
= 100 + 40 + 5 = 145`
  },
  {
    id: 2,
    question: "Programowanie strukturalne i niestrukturalne.",
    answer: `Programowanie strukturalne i niestrukturalne to dwa różne podejścia do budowy programu, które
znacząco wpłynęły na rozwój informatyki.​
Programowanie niestrukturalne, stosowane na początku historii programowania, opierało się na
liniowym wykonywaniu kodu oraz swobodnych skokach w kodzie (instrukcja “goto”). Powodowało
to powstawanie chaotycznego „spaghetti code”, który był trudny w analizie, testowaniu i
poprawianiu błędów, ponieważ kod nie miał uporządkowanej, logicznej struktury.
W odpowiedzi na te problemy pojawiło się programowanie strukturalne, które opiera się na:
sekwencji, instrukcjach warunkowych oraz pętlach. Dzięki eliminacji niekontrolowanych skoków
kod staje się czytelniejszy, bardziej przewidywalny i znacznie łatwiejszy w utrzymaniu.
Programowanie strukturalne stało się podstawą współczesnych metod tworzenia oprogramowania i
fundamentem dla programowania proceduralnego, obiektowego oraz wielu współczesnych języków.`
  },
  {
    id: 3,
    question: "Rola wskaźników w programowaniu.",
    answer: `Rolą wskaźnika jest przechowywanie adresu do pamięci konkretnej zmiennej.​
Dzięki wskaźnikom, możliwa jest dynamiczna alokacja i dealokacja pamięci, co jest kluczowe, gdy
nie są znane rozmiary potrzebnych zasobów przed startem programu.
Wskaźniki pozwalają też na manipulację wartościami zmiennych przez dowolną funkcję, co pozwala
działać poza zakresem funkcji.`
  },
  {
    id: 4,
    question: "Rola w programowaniu instrukcji pętli, rodzaje instrukcji pętli.",
    answer: `Pętle pozwalają na wykonanie danej instrukcji, ciągu instrukcji wiele razy, w zależności od
spełnienia określonego warunku. Ze względu na warunki, są 3 główne rodzaje pętli: for, while,
do-while. For oraz while sprawdzają warunek przed wykonaniem instrukcji, do-while wykona
przynajmniej raz instrukcje przed sprawdzeniem.`
  },
  {
    id: 5,
    question: "Jakie jest znaczenie asercji początkowej i końcowej algorytmu?",
    answer: `Asercja początkowa i końcowa algorytmu służą do formalnego udowadniania poprawności i
specyfikacji algorytmu.
Asercja początkowa określa warunki, które muszą być spełnione przed rozpoczęciem pracy
algorytmu, najczęściej warunki dla danych wejściowych.
Asercja końcowa określa przewidywany warunki, jakie powinny być spełnione po zakończeniu
działania algorytmu.​
Dzięki nim można udowodnić poprawność algorytmu i łatwiej testować programy pod kątem
poprawności logicznej.`
  },
  {
    id: 6,
    question: "Czym różni się rekurencja ogonowa od bezogonowej?",
    answer: `Rekurencja ogonowa może zostać skutecznie zastąpiona przez instrukcje pętli. Ostatnią instrukcją
jest wywołanie rekurencyjne.​
Rekurencja bezogonowa wykonuje po wywołaniu dodatkowe instrukcje. W związku z tym nie może
zostać zoptymalizowana. Blokuje zasoby stosu do czasu wykonania wywołań.`
  },
  {
    id: 7,
    question: "Porównaj własności statycznej tablicy indeksowanej z jednokierunkową listą wiązaną.",
    answer: `
Tablica ma z góry określony, jednorodny rozmiar w pamięci. Lista jest tworzona dynamicznie, przez
co jest większych rozmiarów, a elementy mogą być w dowolnym miejscu w pamięci.​
Dostęp do elementów w tablicy jest prosty, poprzez indeks elementu, w liście należy przejść przez
wszystkie kolejne elementy, aż dojdziemy do wybranego.​
Usuwanie czy dodawanie elementów z kolei jest prostsze w liście, ponieważ sprowadza się do
zmiany wskaźników, w tablicy wymaga przesunięcia kolejnych elementów.`
  },
  {
    id: 8,
    question: "Jakie znasz algorytmy przeszukiwania drzew binarnych?",
    answer: `Przeszukiwanie drzewa binarnego polega na odwiedzeniu wszystkich węzłów w określonej
kolejności. Dwa główne podejścia to przeszukiwanie wgłąb (Depth-First Search) oraz
przeszukiwanie wszerz (Breadth-First Search).​
Przeszukując wgłąb (DFS) schodzimy jak najgłębiej w drzewo, a następnie wracamy do poprzednich
węzłów. Wyróżnia się tutaj 3 warianty:​
- pre-order: węzeł, lewo, prawo;​
- in-order: lewo, węzeł, prawo;​
- post-order: lewo, prawo, węzeł.​
Przeszukiwanie wszerz działa poziomami, zaczyna się od korzenia, a następnie przechodzi się do
węzłów na każdym kolejnym poziomie, od lewej, używając kolejki.
Znam 3 algorytmy, pre-order, in-order, post-order. Sprowadzają się one do przechodzenia przez
kolejne węzły drzew w określonej kolejności.


Zasada działania dla przykładowego drzewa:​
Zaczynamy przeszukiwania od korzenia A, następnie przechodzimy w lewo do B i finalnie od C.​
W zależności od algorytmy nasze przejście zapiszemy:​
- pre: ABC​
- in: BAC​
- post: BCA​
Jeśli drzewo jest większe dzielimy je na podrzewa i rozpisujemy rekurencyjnie przejścia.`
  },
  {
    ...
    answer: `Stos i kolejka to ustrukturyzowane sposoby zapisu danych. Różnią się one przede wszystkim
sposobem obsługi elementów.​
W LIFO stosie, element, który został dopisany jako ostatni, jest przeznaczony do wykorzystania jako
pierwszy (Last In First Out).​
W FIFO kolejce, element dopisany ostatni, jest użyty jako ostatni (First In First Out).`
  },
  {
    ...
    answer: `Złożoność obliczeniowa algorytmu określa szacowane zasoby - czas i pamięć - jakie są niezbędne do
wykonania algorytmu.​
Złożoność jest określana za pomocą notacji O(n), nie w jednostce czasu, lecz w zależności od danych
wejściowych.​
Np. czas przejścia po wszystkich n-elementach tablicy określa się jako O(n), a posortowanie takiej
tablicy w słabym przypadku może wynieść n^2.​
Do złożoności obliczeniowej można również podać złożoność pamięciową, czyli ilość zasobów
pamięci jakie zaangażuje algorytm na swoje potrzeby.`
  },
  {
    id: 11,
    question: "Rodzaje, charakterystyka i przeznaczenie rejestrów procesora.",
    answer: `Rejestry procesora to najszybsza pamięć w komputerze, umieszczona bezpośrednio w CPU, o bardzo
małej pojemności. Przechowują dane, adresy oraz informacje sterujące potrzebne podczas
...
- rejestr flag (rejestr stanu) - zawiera informacje opisujące stan procesora, wpływa na wykonywania
instrukcji warunkowych;​
- wskaźnik stosu - przechowuje adres wierzchołka stosu;​
- rejestry specjalne - kontrolne i segmentowe, odpowiadają za tryby pracy procesora i zarządzanie
pamięcią.`
  },
  {
    id: 12,
    ...
    answer: `
Pamięć wirtualna to technika dająca każdemu procesowi wrażenie posiadania własnej, dużej
przestrzeni adresowej, niezależnie od faktycznej ilości pamięci RAM. Umożliwia uruchamianie
programów większych niż dostępna pamięć fizyczna oraz zapewnia izolację i ochronę między
procesami.
Przestrzeń adresowa procesu dzielona jest na strony, które mapowane są na fizyczne ramki w RAM.
Tłumaczeniem adresu wirtualnego na fizyczny zajmuje się Memory Management Unit (MMU),
korzystając z tablicy stron. Dla przyspieszenia translacji stosuje się Translation Lookaside Buffer
(TLB) - szybką pamięć podręczną przechowującą ostatnio używane odwzorowania.​
Jeśli szukanego adresu nie ma w TLB, MMU sięga do tablicy stron. Jeśli strona nie znajduje się w
RAM, dochodzi do page fault i system operacyjny ładuje ją z dysku do wolnej ramki i wznawia
wykonanie instrukcji.`
  },
  {
    ...
    answer: `
Szybkość wykonywania rozkazów przez procesor zależy od kilku czynników.​
Najważniejsze są: częstotliwość taktowania rdzenia, która określa liczbę cykli na sekundę, oraz ilość
cykli, wymaganych do wykonania jednej instrukcji (Cycles Per Instruction). instrukcji, które może
wykonać jednostka w cyklu. Oba czynniki wyznaczają przepustowość procesora.
Ważna jest mikroarchitektura, wprowadzone techniki wpływające na wydajność: potokowanie podział instrukcji na etapy realizowane równolegle; wykonywanie poza kolejnością - pozwala omijać
przestoje; przewidywanie skoków - redukuje opóźnienia, ale nietrafione skutkuje koniecznością
opróżnienia potoku.
Ostatnimi czynnikami są: pamięć podręczna (cache), która redukuje czas oczekiwania danych oraz
liczba rdzeni oraz wątków procesora, które umożliwiają obsługę instrukcji równolegle.`
  },
  {
    id: 14,
    question: "Szeregowanie procesów, jakie algorytmy są stosowane w szeregowaniu procesów w systemach operacyjnych.",
    answer: `
Szeregowanie procesów to mechanizm systemu operacyjnego polegający na wyborze procesu, który
w danym momencie otrzyma dostęp do procesora.
Algorytmy dzielą się na niewywłaszczające, gdzie procesor działa do zakończenia lub zablokowania
oraz wywłaszczające, gdzie system może odebrać CPU działającem procesowi.
...
- proces otrzymuje kwant czasu, po upływie którego jest wywłaszczany;​
- szeregowanie priorytetów.
Stosuje się wielopoziomowe kolejki łączące algorytmy w celu równoważenia wydajności.`
  },
  {
    id: 15,
    question: "Synchronizacja procesów, jakie metody synchronizacji procesów można",
    answer: `wykorzystać w programach komputerowych.
Synchronizacja procesów zapewnia poprawną współpracę wielu procesów lub wątków
korzystających z tych samych zasobów oraz chroni sekcje krytyczne przed jednoczesnym dostępem.​
Stosuje się różne metody, w tym semafory, muteksy, monitory i zmienne warunkowe, które
pozwalają uniknąć wyścigów, zakleszczeń i innych błędów współbieżności. W nowoczesnym
programowaniu stosuje się także algorytmy bezblokujące (lock-free), które zwiększają wydajność i
...
Zmienna warunkowa - pozwala wątkom czekać na określone zdarzenie.`
  },
  {
    ...
    answer: `
Zakleszczenie to sytuacja, w której procesy wzajemnie czekają na zasoby i żaden nie może
kontynuować pracy. Występuje, gdy spełnione są cztery warunki Coffmana: wzajemne wykluczanie;
przetrzymywanie i oczekiwanie; brak wywłaszczenia; oczekiwanie cykliczne.​
...
- unikanie przy użyciu algorytmu bankiera - przydzielanie zasobów tylko, gdy system pozostaje w
stanie bezpiecznym;​
- wykrywanie i usuwanie - oparte na analizie grafu przydziału zasobów, system usuwa zakleszczenie
poprzez wywłaszczenie zasobu lub zabicie procesu; ​
- ignorowanie problemu - w systemach ogólnego przeznaczenia, zakłada się rzadkie występowanie
problemu.`
  },
  {
    id: 17,
    ...
    answer: `
Przydział pamięci wykonuje się w oparciu o segmentację, czyli podział pamięci procesu na logiczne segmenty o
zmiennym rozmiarze, odpowiadający strukturze programu: kod, dane czy stos.​
Przydział wolnego miejsca w pamięci fizycznej dla segmentu odbywa się według strategii:​
- First Fit - przydzielany jest pierwszy, wystarczająco duży, wolny obszar - rozwiązanie szybkie, ale prowadzące do
fragmentacji na początku pamięci;​
- Best Fit - przydzielany jest najmniejszy pasujący obszar - minimalizowane są straty miejsca w pamięci, ale zostawiane
jest wiele małych fragmentów;​
- Worst Fit - przydzielany jest największy dostępny obszar - zostawiane są duże resztki, które mogą być ponownie
używane.​
Segmentacja prowadzi do fragmentacji zewnętrznej, potrzebne są określone ciągłe fragmenty wolnej przestrzeni pamięci.​
Stosuje się kompaktowanie lub łączy segmentację ze stronicowaniem - segmenty są dzielone na strony stałego rozmiaru,
które mogą być rozmieszczone w nieciągłych ramkach pamięci, eliminując fragmentację zewnętrzną. Umożliwia ona
również współdzielenie segmentów pamięci między procesami.`
  },
  {
    id: 18,
    question: "System plików i jego realizacja w systemie komputerowym.",
    answer: `System plików to mechanizm systemu operacyjnego umożliwiający organizację, przechowywanie i
odczyt danych na nośnikach pamięci masowej.​
System operacyjny wykorzystuje system plików do mapowania logicznej struktury plików na
fizyczną organizację danych na nośniku. Zapewnia strukturę katalogową, zarządzanie metadanymi
plików oraz kontrolę dostępu. Pliki mogą być alokowane w sposób ciągły, łańcuchowy lub
indeksowy. Współczesne systemy stosują mechanizmy zarządzania wolną przestrzenią oraz
dziennikowanie w celu zapewnienia spójności danych.​
Przykładami systemów plików są NTFS, FAT32, ext4.`
  },
  {
    id: 19,
    question: "Sposób wyznaczania trasy dla przesłania pakietu przy wykorzystaniu",
    answer: `protokołów routingu dynamicznego.
Routing dynamiczny polega na automatycznym wyznaczaniu tras poprzez wymianę informacji
między routerami.
Stosuje się trzy typy protokołów:​
- Distance vector (np. RIP) - każdy router zna tylko odległości do sąsiadów i wymienia tablice
routingu z sąsiadami. Trasy wyznaczane algorytmem Bellmana-Forda, gdzie metryką jest liczba
hopów;​
- Link state (np. OSPF) - każdy router zna pełną topologię sieci, rozsyła informacje o stanie swoich
łączy do wszystkich routerów i wyznacza trasy algorytmem Dijkstry, metryką jest kosz łącza.​
- Path vector (np. BGP) - stosowany między systemami autonomicznymi, analizuje całą ścieżkę AS,
co pozwala uniknąć pętli i stosować polityki routingu.
Router wybiera najlepszą trasę na podstawie metryki i aktualizuje tablice routingu w procesie
konwergencji, czyli procesie w którym wszystkie routery osiągają spójny obraz topologii.`
  },
  {
    id: 20,
    question: "Porównanie protokołów routingu dynamicznego: RIP oraz OSPF.",
    answer: `RIP (Routing Information Protocol) i OSPF (Open Shortest Path First) to protokoły routingu
dynamicznego stosowane do wyznaczania tras w sieciach IP.
RIP - protokół typu distance vector - routery wymieniają się tablicami routingu z sąsiadami, trasy
wyznaczane są algorytmem Bellmana-Forda, a metryką jest liczba przeskoków (z limitem 15, co
ogranicza zasięg). RIP charakteryzuje się wolną konwergencją - proces aktualizacji obrazu topologi
we wszystkich routerach - i słabą skalowalnością, dlatego przeznaczony jest do małych sieci.​
OSPF - protokół typu link state - każdy router posiada pełną wiedzę o topologii sieci, trasy
wyznaczane są algorytmem Dijkstry na podstawie kosztu łącza. OSPF obsługuje podział sieci na
obszary, zapewnia szybką konwergencję i dobrą skalowalność, przeznaczony do większych sieci.`
  },
  {
    id: 21,
    question: "Rodzaje i charakterystyka metod unikania kolizji stosowane w sieciach",
    answer: `przewodowych oraz bezprzewodowych.
Kolizje występują gdy kilka urządzeń jednocześnie nadaje przez wspólne medium transmisyjne.


W sieciach przewodowych stosowano CSMA/CD — urządzenie nasłuchuje medium przed
nadaniem, wykrywa kolizję podczas transmisji i ponawia ją po losowym czasie. W nowoczesnych
sieciach Ethernet z przełącznikami każde łącze tworzy osobną domenę kolizji, więc CSMA/CD jest
praktycznie nieużywane.
W sieciach bezprzewodowych wykrycie kolizji jest niemożliwe, bo urządzenie nie może
jednocześnie nadawać i nasłuchiwać. Stosuje się CSMA/CA — urządzenie nasłuchuje medium, a
przed nadaniem odczekuje losowy czas, starając się kolizji uniknąć. Uzupełnia go mechanizm
RTS/CTS, który rozwiązuje problem ukrytej stacji: dwa urządzenia mogą nie słyszeć siebie
nawzajem, ale obie komunikują się z tym samym punktem dostępowym. RTS/CTS rezerwuje
medium przed właściwą transmisją.`
  },
  {
    id: 22,
    question: "Rodzaje i charakterystyka technologii stosowanych w sieciach",
    answer: `rozległych.
Sieci WAN łączą odległe sieci komputerowe na dużych obszarach geograficznych. Stosuje się w nich
dwa podstawowe modele transmisji.
Komutacja obwodów rezerwuje dedykowane połączenie na czas trwania sesji — zasoby są zajęte
niezależnie od tego czy dane są aktualnie przesyłane. Stosowana w tradycyjnej telefonii.
Komutacja pakietów dzieli dane na pakiety przesyłane niezależnie, współdzieląc zasoby sieci —
stanowi podstawę działania Internetu.
Technologie stosowane w sieciach WAN: MPLS kieruje ruchem na podstawie etykiet zamiast analizy
adresów IP, co zapewnia przewidywalność i jakość usług — szeroko stosowane przez operatorów
telekomunikacyjnych; łącza światłowodowe zapewniają wysoką przepustowość na duże odległości i
stanowią fizyczną podstawę nowoczesnych sieci WAN; VPN umożliwia bezpieczne tunelowanie
ruchu przez publiczną infrastrukturę sieciową.`
  },
  {
    id: 23,
    question: "Charakterystyka stosu protokołów TCP/IP.",
    answer: `Stos protokołów TCP/IP to podstawowy model komunikacji w Internecie, składający się z czterech
warstw.
Warstwa dostępu do sieci - odpowiada za fizyczne przesyłanie danych w sieci lokalnej - obejmuje
protokoły takie jak Ethernet czy Wi-Fi.
Warstwa internetowa - odpowiada za adresowanie i routing pakietów między sieciami przy użyciu
protokołu IP.
Warstwa transportowa - zapewnia komunikację między aplikacjami. TCP jest protokołem
połączeniowym, zapewniającym niezawodne dostarczenie danych z potwierdzeniami. UDP jest
bezpołączeniowy, szybszy, bez gwarancji dostarczenia - stosowany tam gdzie liczy się szybkość, np.
w transmisji video.
Warstwa aplikacji - obejmuje protokoły używane bezpośrednio przez programy, takie jak HTTP, FTP czy SMTP.`
  },
  {
    id: 24,
    question: "Istota działania zdalnego wywoływania procedur (RPC) - na wybranym przykładzie.",
    answer: `
RPC (Remote Procedure Call) umożliwia wywoływanie procedur znajdujących się na zdalnym
komputerze w sposób zbliżony do wywołania lokalnego — programista wywołuje funkcję nie
martwiąc się szczegółami komunikacji sieciowej.
Mechanizm działa następująco: po stronie klienta przechwytuje się wywołanie (stub klienta fragment kodu, działający jako zamiennik procedury); serializuje parametry (marshalling); przesyła
żądanie przez sieć do serwera. Po stronie serwera odbiera się żądanie (stub serwera); deserializuje
parametry i wywołuje właściwą procedurę. Wynik jest serializowany i odesłany do klienta, gdzie
odpakowuje się go i zwraca do wywołującego kodu.
Przykładem jest aplikacja bankowa, gdzie klient wywołuje funkcję SprawdźSaldo() — lokalny stub
serializuje numer konta, przesyła żądanie do serwera centralnego, który wykonuje procedurę i odsyła
wynik.`
  },
  {
    id: 25,
    question: "Czym jest paradygmat programowania? Wymień kilka paradygmatów",
    answer: `programowania.
Paradygmat programowania jest to sposób podejścia do tworzenia programu komputerowego.
Określa on fundamentalne mechanizmy i podejścia - najczęściej w danym języku programowania.
Obejmuje sposoby organizacji kodu i rozwiązywania problemów w określonym stylu.​
Najważniejsze paradygmaty: imperatywny; funkcyjny; obiektowy; logiczne; deklaratywne.
Współczesne języki często łączą wiele paradygmatów jednocześnie`
  },
  {
    id: 26,
    question: "Zdefiniuj pojęcie klasy w programowaniu obiektowym i omów jej",
    answer: `składowe.
Klasy w programowaniu obiektowym to wzorzec, na podstawie którego tworzone są obiekty definiuje ich strukturę i zachowanie
Składowe klasy:​
...
Klasu umozliwiają organizację kodu oraz realizację zasad programowania obiektowego, takie jak
enkapsulacja i dziedziczenie.`
  },
  {
    id: 27,
    question: "Na czym polega zjawisko polimorfizmu w programowaniu",
    answer: `obiektowym?
Polimorfizm (wielopostaciowość) w programowaniu obiektowym odnosi się on do zdolności
obiektów do przyjmowania wielu form - dzięki czemu obiekty mogą reagować na tę samą metodę w
różny sposób. Interfejs może mieć różne implementacje, zależne od typu obiektu.
Wyróżnia się:​
...
Przykład: klasy Pies i Kot dziedziczą z klasy Zwierzę, obie implementują metodę wydajDźwięk(),
która daje różne efekty w zależności od zwierzęcia.
Dzięki polimorfizmowi kod staje się bardziej elastyczny i łatwiejszy do rozbudowy.`
  },
  {
    id: 28,
    question: "Co to są metody anonimowe i wyrażenia strzałkowe (lambda) w",
    answer: `programowaniu dynamicznym?
Metoda anonimowa (funkcja anonimowa) to funkcja bez nazwy, definiowana i używana
bezpośrednio w miejscu wywołania, najczęściej jako argument przekazywany do innej metody.
Wyrażenie strzałkowe (lambda) to skrócona, bardziej zwięzła forma zapisu funkcji anonimowej,
korzystająca z operatora => (stąd nazwa strzałkowe).
Umożliwiają przekazywanie funkcji jako argumentów oraz stosowanie ich w operacjach na danych,
takich jak filtrowanie, mapowanie czy sortowanie. Dzięki nim kod staje się krótszy i bardziej
elastyczny, szczególnie w programowaniu funkcyjnym i dynamicznym.`
  },
  {
    id: 29,
    question: "Scharakteryzuj krótko platformę .NET.",
    answer: `Platforma .NET to środowisko programistyczne stworzone przez Microsoft, umożliwiające tworzenie
aplikacji na różne platformy systemowe. Jej głównym elementem jest środowisko uruchomieniowe
CLR (Common Language Runtime), które zarządza pamięcią, obsługuje wyjątki i wykonuje kod
pośredni. Platforma udostępnia także wspólny system typów (CTS) oraz bogate biblioteki klas.
Dzięki temu pozwala tworzyć aplikacje w różnych językach, takich jak C#, F# czy VB.NET, i
uruchamiać je w zunifikowanym środowisku.`
  },
  {
    id: 30,
    question: "Różnice pomiędzy grafiką rastrową i wektorową oraz ich zastosowania.",
    answer: `Grafika rastrowa zapisuje obraz jako siatkę pikseli o określonych wymiarach i barwach. Jest zależna
od rozdzielczości — przy powiększaniu traci jakość. Stosowana głównie do zdjęć i obrazów o
złożonej kolorystyce. Przykładowe formaty: JPEG, PNG, BMP.
Grafika wektorowa opisuje obraz za pomocą obiektów matematycznych: linii, krzywych i figur z
określonymi parametrami i kolorami. Jest niezależna od rozdzielczości i może być skalowana bez
utraty jakości. Stosowana do logotypów, ikon, ilustracji i grafiki technicznej. Przykładowe formaty:
SVG, AI, EPS.`
  },
  {
    id: 31,
...
historycznych); bezpieczeństwo; nadmierne zaufanie.`
  },
  {
    ...
    answer: `Internet rzeczy (IoT) to sieć fizycznych urządzeń wyposażonych w sensory, oprogramowanie i łączność
sieciową, które zbierają i wymieniają dane. Architektura IoT obejmuje warstwę urządzeń, bramę
(gateway) i chmurę; komunikacja odbywa się przez dedykowane protokoły.
Szanse:
...
Zagrożenia:
●​ bezpieczeństwo - urządzenia IoT często mają słabe zabezpieczenia i bywają przejmowane do ataków
- botnet Mirai (2016) wykorzystał miliony kamer i routerów do przeprowadzenia największego
wówczas ataku DDoS;
●​ prywatność - masowe zbieranie danych o zachowaniu użytkowników w czasie rzeczywistym;
●​ brak standardów - fragmentacja protokołów i brak jednolitych standardów bezpieczeństwa utrudnia
interoperacyjność;
...
Edge computing - przetwarzanie danych blisko źródła, na urządzeniu lub bramie zamiast w chmurze jest odpowiedzią na problemy latencji i przepustowości w IoT.`
  },
  {
    ...
    answer: `Autonomia to zdolność do samodzielnego działania i podejmowania decyzji. W kontekście
technologii, oznacza zachowanie kontroli nad: swoimi danymi; komunikacją; używanym
oprogramowaniem; dostępem do informacji; zależnością od platform i dostawców usług.
Podstawowe sposoby zachowania autonomii to m.in.:
-​
ochrona prywatności i danych, poprzez stosowanie silnych haseł; uwierzytelnianie
wieloskładnikowe; ograniczanie ilości publikowanych danych osobowych;
-​
świadome korzystanie z usług internetowych: czytanie polityk prywatności; kontrolowanie
uprawnień aplikacji; dywersyfikacja dostawców usług;
-​
korzystanie z otwartych standardów i oprogramowania open source: co umożliwia
kontrolę oraz zmniejsza zależność od zamkniętych ekosystemów;
-​
komunikowanie się w bezpieczny sposób, używając szyfrowania, np. HTTPS czy
...
mając świadomość zagrożeń, aktualizując wiedzę oraz dobre praktyki.`
  },
  {
    ...
    answer: `
Licencja oprogramowania określa zasady użytkowania, modyfikacji i dystrybucji aplikacji. W
nowoczesnej architekturze, np. systemów chmurowych, mikroserwisów, czy aplikacji SaaS, licencje
mają istotny wpływ na sposób integracji komponentów i dystrybucji oprogramowania.​
Podstawowe rodzaje licencji to:
-​
licencje własnościowe.​
Kod źródłowy jest zamknięty, a użytkownik ma prawa użycia zgodne z umową producenta.
-​
licencje open source.​
Pozwalają na dostęp do kodu źródłowego i jego modyfikację. Wyróżnia się licencje: permisywne,
które pozwalają na szerokie wykorzystanie kodu (MIT, Apache); copyleft, które wymagają
udostępnienia kodu pochodnego na tej samej licencji (GPL).`
  },
];
